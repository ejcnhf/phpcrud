 ...
 if(isset($_POST['update'])){
    ...
    if($con->updateUser($user_id, $firstname, $lastname, $birthday, $sex, $username, $password)){
        if ($con->updateUserAddress($user_id,$street,$barangay,$city,$province)){
         ...
         $error = "Error occured while updating user adress please try again";
        }
     } else{
            $error = "Error occured while updating user infromation. please try again";
        }
    }
 }
     ...
     <div class="card mt-4">
       ...
       <div class="card-body">
         ...
         <div class="form-group">
           ...
           <input type="password" class="form-control" name="cpass" placeholder="Enter password"value="<?php echo $data['Pass_word'];?>">
         </div>
       </div>
     </div>
